USE [master]
GO
IF OBJECT_ID('dbo.sp_WhatIsRunning') IS NULL
	EXEC ('CREATE PROCEDURE [dbo].[sp_WhatIsRunning] AS RETURN 0;');
GO
...
/* 
	...
	Sproc Name:			dbo.sp_WhatIsRunning
	Current Version:	0.3
	Description: 1. Checks the Active connections
		...
		usage:  exec sp_WhatIsRunning -- This display the Active connections by default with order by Logical_Reads and CPU%, Diskspace, Lead Blocker, Long runngin connections
				Select parameters with values as below:
				exec sp_WhatIsRunning @p_Option = 0 - All, 1 - Connections, 2 - CPU %, 3 - Diskspace, 4 - Lead Blocker, 5 - Backup/Rollback status.
		...
		Example:									
			...
			exec sp_WhatIsRunning -- This display the Active connections by default with order by Logical_Reads and CPU%, Diskspace, Lead Blocker, Long runngin connections
			exec sp_WhatIsRunning @p_Option = 0  -- This display the Active connections by default with order by Logical_Reads and CPU%, Diskspace, Lead Blocker, Long running connections
			exec sp_WhatIsRunning @p_Option = 1	 -- Active connections

			exec sp_WhatIsRunning @p_Option = 1, @p_Status = 'All', @p_Orderby = 3		-- Connection with Active and Sleeping and Order by SPID
			exec sp_WhatIsRunning @p_Option = 1, @p_Status = 'ACTIVE', @p_Orderby = 1	-- Connection with Active and Order by Logical_Reads desc
			exec sp_WhatIsRunning @p_Option = 1, @p_Status = 'SLEEPING', @p_Orderby = 1	-- Connection with Sleeping and Order by Logical_Reads desc
			exec sp_WhatIsRunning @p_Option = 2  -- CPU usage % and Memory Usage %
												( This feature is applies to SQL Server 2008 plus higher version )
			exec sp_WhatIsRunning @p_Option = 3  --	Displays the capacity on the server and databases free and used
			...
			exec sp_WhatIsRunning @p_Option = 4	 --	Lead Blocker list only the waittime greater than 60 seconds 
			exec sp_WhatIsRunning @p_Option = 5	 -- Long running connections: just ass Backup/Rollback, DBCC TABLE CHECK/Shrinkfile status with estimation time of completion with percentage'
			exec sp_WhatIsRunning @p_Option = 6	 -- AlwaysOn availability Group status
			exec sp_WhatIsRunning @p_Option = 7	 -- Mirroring status
		...
		** Note:
		...
		Store Procedure Parameters:
			sp_WhatIsRunning 
					...
					@p_Orderby		varchar(1)
		usage:
			exec sp_WhatIsRunning -- This display the Active connections by default with order by Logical_Reads and CPU%, Diskspace, 
			...
			exec sp_WhatIsRunning 
					...
					, @p_Orderby	= 1 - ''Logical_Reads desc'' ( default )
								  ...
								  4 - ''Order by DBName''
		Example:									
			exec sp_WhatIsRunning ''?''  -- Help
			exec sp_WhatIsRunning -- This display the Active connections by default with order by Logical_Reads and CPU%, Diskspace, 
								   Lead Blocker, Long running connections, AlwaysOn availability status and Mirroring status
			exec sp_WhatIsRunning @p_Option = 0  -- This display the Active connections by default with order by Logical_Reads and CPU%, Diskspace, 
												Lead Blocker, Long running connections, AlwaysOn availability status and Mirroring status
			exec sp_WhatIsRunning @p_Option = 1	 -- Active connections
			...
			exec sp_WhatIsRunning @p_Option = 2  -- CPU and Memory Usage % ( This feature is applies to SQL Server 2008 plus higher version )
			exec sp_WhatIsRunning @p_Option = 3  --	Displays the capacity on the server and databases space used and availability 
												( The Server Capacity feature is applies to SQL Server 2008 plus higher version )
			exec sp_WhatIsRunning @p_Option = 4	 --	Blocking Tree only the waittime greater than 60 seconds 
			exec sp_WhatIsRunning @p_Option = 5	 -- Long running connections: Just as Backup/Rollback status with estimation time of completion with percentage
			exec sp_WhatIsRunning @p_Option = 6	 -- This displays the AlwaysOn availability Group status
			exec sp_WhatIsRunning @p_Option = 7	 -- This displays the Mirroring status
			...
			exec sp_WhatIsRunning 6
			exec sp_WhatIsRunning 7'
...
exec dbo.sp_Kill @p_DbName = 'Youtube_20130710_AudioMusic', @p_Force = 1
					,@p_Verbose = 1

exec msdb..sp_start_job [DBA Log Walk - Restore Youtube_20130710_AudioMusic]
go
