...
declare @table_name nvarchar(500) --= 'usp_get_user_votes';
...
set @sql_string = "
;with cte_group as (
	select	[grouping-key] = (case when client_app_name like 'SQL Job = %' then client_app_name else left(DBA.dbo.normalized_sql_text((case when ltrim(rc.sql_text) like 'exec sp_executesql @statement=N%' 
	...
	group by (case when client_app_name like 'SQL Job = %' then client_app_name else left(DBA.dbo.normalized_sql_text((case when ltrim(rc.sql_text) like 'exec sp_executesql @statement=N%' 
									then substring(ltrim(rtrim(rc.sql_text)),33,len(ltrim(rtrim(rc.sql_text)))-33)
									when ltrim(rc.sql_text) like 'exec sp_executesql%' 
									then substring(ltrim(rtrim(rc.sql_text)),22,len(ltrim(rtrim(rc.sql_text)))-22)
									when rc.sql_text like'%sp_prepexec%'
...
cross apply (select top 1 * from dbo.vw_xevent_metrics rc 
			...
			"+(CASE WHEN @table_name IS NULL THEN "--" ELSE "" END)+"and rc.sql_text like ('%'+@table_name+'%')
			and result = 'OK'
			and q.[grouping-key] = (case when rc.client_app_name like 'SQL Job = %' then rc.client_app_name else left(DBA.dbo.normalized_sql_text((case when ltrim(rc.sql_text) like 'exec sp_executesql @statement=N%' 
									then substring(ltrim(rtrim(rc.sql_text)),33,len(ltrim(rtrim(rc.sql_text)))-33)
									when ltrim(rc.sql_text) like 'exec sp_executesql%' 
									then substring(ltrim(rtrim(rc.sql_text)),22,len(ltrim(rtrim(rc.sql_text)))-22)
									when rc.sql_text like'%sp_prepexec%'
