...
ALTER PROCEDURE [dbg].[usp_sqlwatch_internal_process_checks] @verbose bit = 0
AS
/*
-------------------------------------------------------------------------------------------------------------------
...
IF @verbose = 1
	PRINT 'Declare variables at start';

declare @lines varchar(2000);
set @lines = '-------------------------------------------------------------------------------------------------------------------'+char(10)+char(13);
...
IF @verbose = 1
	PRINT 'Execute procedure [dbo].[usp_sqlwatch_internal_insert_header]';
...
IF @verbose = 1
BEGIN
	PRINT 'Declare cursor cur_rules for [dbo].[sqlwatch_config_check] inner join [dbo].[sqlwatch_meta_check]';
	select 'Declare cursor cur_rules for [dbo].[sqlwatch_config_check] inner join [dbo].[sqlwatch_meta_check]' as RunningQuery,
		  cc.[check_id]
		, cc.[check_name]
...
IF @verbose = 1
		print 'Fetch next from cur_rules, and enter into loop';
...
begin
	IF @verbose = 1
		print '  Inside cur_rules loop. @check_id => '+cast(@check_id as varchar);
...
' + replace(@check_query,'{LAST_CHECK_DATE}',convert(varchar(23),@previous_check_date,121));
		...
		if @verbose = 1
			print 'Try Execute @check_query => '+char(10)+char(13)+@check_query+char(10)+@lines;
	...
	begin catch
		if @verbose = 1
			print 'Catch Execute @check_query => '+char(10)+char(13)+@check_query+char(10)+@lines;
